...
| **Test Command** | `python -m pytest tests/ -q` → **980 passed, 9 skipped, 0 failed in ~2.3s** (at the ROB-07 fix commit) |
...
tests/                    → 35 files, ~11,600 LOC, 980 tests (mocked unit tests)
...
- **No CI** — `.github/workflows/` does not exist; the 980-test suite runs only on maintainer machines
...
5. Run `python -m pytest tests/ -q` before and after any change — 2.3 seconds, no excuse; expect exactly 980 passed / 9 skipped at the ROB-07 fix commit
